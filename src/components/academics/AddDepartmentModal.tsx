...
export default function AddDepartmentModal({ open, onClose }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await api.post('/academics/departments', { name, code, description: description || undefined, headId: headId || undefined });
      onClose();
      router.refresh();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <form onSubmit={submit} className="w-full max-w-md bg-white rounded-lg p-6 shadow">
        ...
        <label className="block mb-4">
          <div className="text-sm text-gray-600">Head (faculty id, optional)</div>
          <input value={headId} onChange={e => setHeadId(e.target.value)} className="mt-1 w-full px-3 py-2 border rounded" />
        </label>
